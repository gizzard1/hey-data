...
class Empleados extends Component
{
    ...
    [
        ...
        'empleado.color_preset' => 'nullable',
    ];
    ...
    public function updateColor($empleadoId, $color)
    {
        try {
            $empleado = Empleado::find($empleadoId);
            if ($empleado) {
                $empleado->color_preset = $color;
                $empleado->save();
                $this->dispatchBrowserEvent('noty', ['msg' => 'Color actualizado con éxito']);
            } else {
                $this->dispatchBrowserEvent('noty-error', ['msg' => 'Empleado no encontrado']);
            }
        } catch (\Throwable $th) {
            $this->dispatchBrowserEvent('noty-error', ['msg' => "Código de error: 123425Empleados"]);
        }
    }
    ...
    protected $listeners = [
        'refresh' => '$refresh',
        'search' => 'searching',
        'Delete','Activate','Deactivate','sendFingerprint','returnPatterns','return64Image'
    ];
    ...
    // }
    private function preprocessImage($imgPath)
    {
        $pythonPath = 'C:\\Users\\Mauricio\\AppData\\Local\\Programs\\Python\\Python313\\python.exe'; // usa doble backslash \\ o slash /
        $scriptPath = base_path('app/Http/Livewire/preprocess.py');
    ...
    public function return64Image($untitledImage)
    {
        if (session('checador')) {
            return;
        }
        if ($untitledImage) {
            // Eliminar encabezado base64
            $image = base64_decode(preg_replace('#^data:image/\w+;base64,#i', '', $untitledImage));
            $salon_id = Auth::user()->salon_id;
            // Guardar temporalmente la imagen
            $filename = 'huella_' . uniqid() . '.png';
            $fp_dir = storage_path('app/public/temp/' . $salon_id);
            $path = $fp_dir . '/' . $filename;
            // Asegurar que el directorio exista
            if (!file_exists($fp_dir)) {
                mkdir($fp_dir, 755, true);
            }
            ...
            // Ejecutar análisis
            $resultado = $this->preprocessImage($path);

            if($resultado){
                $publicUrl = asset("storage/temp/{$salon_id}/{$filename}");
                $this->dispatchBrowserEvent('sendProcessImagePath', ['url' => $publicUrl]);
            }
        }
    }
    public function returnPatterns($accumulatedDistances)
    {
        if (session('checador')) {
            return;
        }
        $w=0.45;
        DB::transaction(function () use ($accumulatedDistances,$w) {
            foreach($accumulatedDistances as $pattern){
                if($pattern['normalizedDistance']>0){
                    $patron = empleado_huella::where('empleado_id',$this->empleado->id)->where('pair_type',$pattern['pair'])->first();
                    if($patron){
                        $new_average=($patron->average*(1-$w))+($pattern['normalizedDistance']*$w);
                        $patron->average=$new_average;
                        $patron->counter+=$pattern['count'];
                        $patron->save();
                    }else{
                        //guardar patron
                        empleado_huella::create([
                            'empleado_id' => $this->empleado->id,
                            'pair_type' => $pattern['pair'],
                            'counter' => $pattern['count'],
                            'average' => $pattern['normalizedDistance'],
                        ]);
                    }
                }
            }
    ...
    private function clear()
    {
        ...
        $this->empleado = new Empleado();
        $this->empleado->color_preset = '#E2BBB4';
        $this->editing = false;
