            ...
            foreach ($items_s as $asignacion) {
                ...
                if ($index !== false) {
                    ...
                    if (!in_array($customer_id, $uniqueCustomerIdsD)) {
                        ...
                        $dataEmpleados[$index]['total_cust'] += 1;
                        $this->total_cust += 1;
                        $uniqueCustomerIdsD[] = $customer_id;  // Se añade el id para descartarlo en un futuro
                    }
    ...
    private function obtenerPorcentajes()
    {
        try {
            foreach ($this->dataEmpleados as &$empleado) {
                ...
                $percent = $total_ingresos > 0 ? (($empleado['total_v'] + $empleado['total_d']) / $total_ingresos) * 100 : 0;
