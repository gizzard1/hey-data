...
class Payment extends Component
{
    ...
    protected $listeners = [
        'refresh' => '$refresh','removeItem','setCustomerId','newMethod' => 'setMethod','newPropina' =>'setTip','cambioData','cambioDataMethods','setReward','dataChange','enviarCliente' => 'recibirClienteNuevo','collectMethod', 'enviarData' => 'recibirData','store' => 'storeDate','GCInMethods','rfcSelected','continueStoring'
    ];
    ...
    public function render()
    {
        try{
            ...
            return view('livewire.payment',compact('methods','propinas'), ['disccount_form' => $this->disccount_form, 'restante' => $this->rest,'empleados' => $this->loadEmployees()
            ]);
        }catch(\Throwable $th){
        ...
        try{
            ...
            foreach($cart as $item){
                ...
                $asignacion = new Asignacion_venta ([
                    ...
                    'current_price' => $item['gross_price'] > $item['sale_price'] ? $item['gross_price'] : $item['sale_price'],
            ...
            $rewardPoints = $this->calculateCustomerPoints($this->totalCart);
            $sale->generated_points = $rewardPoints ? $rewardPoints : $this->generatedPoints();
            $sale->save();
