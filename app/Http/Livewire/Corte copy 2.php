...
class Corte extends Component
{
    ...
    public $cajaChica = 0, $caja_chica,$caja_chica_real, $gastos = 0;
    public $diferencia = 0;
    ...
    protected $listeners = [
        'refresh' => '$refresh','changeCash','changeQtyTerminal','changeQtyTerminalPropina'
    ];
    public function render()
    {
        try{
            $this->diferencia = $this->totalCashReal-$this->caja_chica-$this->efectivoCorte-$this->propinasEfectivo+$this->gastos;
            return view('livewire.caja-corte',[
                'ventasCorte' => $this->ventasCorte,
                'serviciosCorte' => $this->serviciosCorte,
                'efectivoCorte' => $this->efectivoCorte,
                'cardCorte' => $this->cardCorte,
                'tarjetaCorte' => $this->tarjetaCorte,
                'msiCorte' => $this->msiCorte,
                'puntosCorte' => $this->puntosCorte,
                'totalCorte' => $this->totalCorte,
                'totalTips' => $this->totalTips,
                'propinasEfectivo' => $this->propinasEfectivo,
                'propinasTarjeta' => $this->propinasTarjeta,
                'tarjetaCortePropina' => $this->tarjetaCortePropina,
                'propinasMsi' => $this->propinasMsi,
            ]);
        }catch(\Throwable $th){
            $this->dispatchBrowserEvent('noty-error', ['msg' =>  "Código de error: 29464Cortes"] );
        }
    }
    ...
    private function sumMethod($metodo,$qty,$isPropina)
    {
        try{
            ...
            }elseif($metodo->payment_method_id===2){
                if(!$isPropina){
                    $this->cardCorte+=$qty;
                }else{
                    $this->propinasTarjeta+=$qty;
                }
            }elseif($metodo->payment_method_id===3){
                if(!$isPropina){
                    $this->msiCorte+=$qty;
                }else{
                    $this->propinasMsi+=$qty;
                }
            }elseif($metodo->payment_method_id===5){
                $this->puntosCorte+=$qty;
            }else{
                if($metodo->payment_method_id!== 4){
                    $reference = $metodo->metodoPago->Payment_method;
                    // Agrega los nuevos elementos al array existente
                    if(!$isPropina){
                        if(isset($this->tarjetaCorte[$reference])){
                            $this->tarjetaCorte[$reference] += $qty;
                        }else{
                            $this->tarjetaCorte[$reference] = $qty;
                        }
                    }else{
                        if(isset($this->tarjetaCortePropina[$reference])){
                            $this->tarjetaCortePropina[$reference] += $qty;
                        }else{
                            $this->tarjetaCortePropina[$reference] = $qty;
                        }
                    }
    ...
    private function obtenerDatosCorte()
    {
        try{
            ...
            $this->gastos = gasto::where('salon_id',Auth::user()->salon_id)
                ->where('payment_method','Caja chica')
                ->whereBetween('created_at', [$aperturaTime, $now])
                ->sum('total');
            $ventas = venta::where('salon_id',Auth::user()->salon_id)
                ->whereBetween('created_at', [$aperturaTime, $now])
                ->where(function($query) {
                    $query->where('status', 'Pagada')
                          ->orWhere('status', 'Pendiente');
                })
                ->with('abonos','details','metodosPago.metodoPago','propinas.metodoPago')->get();
            $citas = cita::where('salon_id', Auth::user()->salon_id)
                ->whereBetween('updated_at', [$aperturaTime, $now])
                ->where(function($query) {
                    $query->where('status', 'Pagada')
                          ->orWhere('status', 'Pendiente');
                })
                ->with('abonos', 'details', 'metodosPago.metodoPago', 'propinas.metodoPago')
                ->get();
            $this->ventasCorte = $this->acumularTransacciones($ventas,0);
            $this->serviciosCorte = $this->acumularTransacciones($citas,1);
            $this->totalCorte=$this->ventasCorte+$this->serviciosCorte;
        }catch(\Throwable $th){
    ...
    public function corteCaja()
    {
        ...
        $this->setTerminalesReales();
    }
    private function setTerminalesReales()
    {
        try{
            $this->tarjetaCorteReal = $this->tarjetaCorte;
            $this->tarjetaCortePropinaReal = $this->tarjetaCortePropina;
            $this->tarjetaCorteReal = $this->setQtyArray($this->tarjetaCorteReal,0);
            $this->tarjetaCortePropinaReal = $this->setQtyArray($this->tarjetaCortePropinaReal,0);
        }catch(\Throwable $th){
            $this->dispatchBrowserEvent('noty-error', ['msg' =>  "Código de error: 285334Cortes"] );
        }
    }
    public function changeQtyTerminal($terminal,$qty)
    {
        $this->tarjetaCorteReal = $this->searchTerminal($this->tarjetaCorteReal,$terminal,$qty);
        $this->totalCorteReal = $this->updateTotal($this->tarjetaCorteReal,$this->totalCashReal,$this->totalNFreal,$this->totalCardReal);
        $this->emit('refresh');
    }
    public function changeQtyTerminalPropina($terminal,$qty)
    {   
        $this->tarjetaCortePropinaReal = $this->searchTerminal($this->tarjetaCortePropinaReal,$terminal,$qty);
        $this->totalTipsReal=$this->updateTotal($this->tarjetaCortePropinaReal,$this->propinasEfectivoReal,$this->propinasMsiReal,$this->propinasCardReal);
        $this->emit('refresh');
    }
    ...
    public function changeCash($type,$qty)
    {
        try{
            ...
            if($type=='incomes'){
                ...
                $this->propinasEfectivoReal = $qty;
                $this->totalTipsReal=$this->updateTotal($this->tarjetaCortePropinaReal,$this->propinasEfectivoReal,$this->propinasMsiReal,$this->propinasCardReal);
            }elseif($type=='msi'){
                $this->totalNFreal = $qty;
                $this->totalCorteReal=$this->updateTotal($this->tarjetaCorteReal,$this->totalCashReal,$this->totalNFreal,$this->totalCardReal);
            }elseif($type=='msiPropinas'){
                $this->propinasMsiReal = $qty;
                $this->totalTipsReal=$this->updateTotal($this->tarjetaCortePropinaReal,$this->propinasEfectivoReal,$this->propinasMsiReal,$this->propinasCardReal);
            }elseif($type=='card'){
                $this->totalCardReal = $qty;
                $this->totalCorteReal=$this->updateTotal($this->tarjetaCorteReal,$this->totalCashReal,$this->totalNFreal,$this->totalCardReal);
            }elseif($type=='cardPropinas'){
                $this->propinasCardReal = $qty;
                $this->totalTipsReal=$this->updateTotal($this->tarjetaCortePropinaReal,$this->propinasEfectivoReal,$this->propinasMsiReal,$this->propinasCardReal);
            }
        }catch(\Throwable $th){
    ...
    private function updateTotal($array,$cash,$msi,$card)
    {
        try{
            $total = 0;
            foreach($array as $terminal => $qty)
            {
                if(is_numeric($qty)){
                    $total+=$qty ?? 0;
                }
            ...
            $this->dispatchBrowserEvent('noty-error', ['msg' =>  "Código de error: 329336Cortes"] );
        }

    }
    private function searchTerminal($array,$nombreT,$cant)
    {
        try{
            foreach($array as $terminal => $qty)
            {
                if($terminal == $nombreT){
                    $array[$terminal] = $cant ?? 0;
                }
            }
            return $array;
        }catch(\Throwable $th){
            $this->dispatchBrowserEvent('noty-error', ['msg' =>  "Código de error: 351337Cortes"] );
        }
    }
    private function setQtyArray($array,$cant)
    {
        try{
            foreach($array as $terminal => $qty)
            {
            ...
            $this->dispatchBrowserEvent('noty-error', ['msg' =>  "Código de error: 365338Ventas"] );
        }
    }
    ...
    private function setMethodsCorte($method,$qty,$corte,$reference)
    {
        try{
            $method->payment_method = $reference;
            $method->qty = $qty;
            $method->caja_corte_id = $corte->id;
            $method->save();
        }catch(\Throwable $th){
            $this->dispatchBrowserEvent('noty-error', ['msg' =>  "Código de error: 416339Cortes"] );
        }
    }
    private function setCorte()
    {
        try{
            ...
            $corte->gastos = $this->gastos;
            $corte->save();
            foreach($this->tarjetaCortePropina as $reference => $qty){
                $metodo_corte = new metodo_propina_corte;
                $metodo_corte->is_real = false;
                $this->setMethodsCorte($metodo_corte,$qty,$corte,$reference);
            }
            foreach($this->tarjetaCorte as $reference => $qty){
                $metodo_corte = new metodo_pago_corte;
                $metodo_corte->is_real = false;
                $this->setMethodsCorte($metodo_corte,$qty,$corte,$reference);
            }
            foreach($this->tarjetaCortePropinaReal as $reference => $qty){
                $metodo_corte = new metodo_propina_corte;
                $metodo_corte->is_real = true;
                $this->setMethodsCorte($metodo_corte,$qty,$corte,$reference);
            }
            foreach($this->tarjetaCorteReal as $reference => $qty){
                $metodo_corte = new metodo_pago_corte;
                $metodo_corte->is_real = true;
                $this->setMethodsCorte($metodo_corte,$qty,$corte,$reference);
            }
