...
class Citas extends Component
{
    ...
    public function mount($action=null,$pestaña=null,$cita_id=null)
    {
        ...
        if($cita_id){
            ...
            session()->put('returnCustomersView', true);
            session()->save();
        }
    }
    ...
    protected $listeners = [
        ...
        'StoreReview','editar','updateQty','changeWindow','deleteMov','aperturaCaja',
        ...
        'removeItem','setCustomerId','Store','setMethod','setTip'=>'newPropina','newPropina','setReward','newMethod','filterEmployee','recorrido','productAdded','rfcSelected',
        // Acciones con teclas
    ...
    public function render()
    {
        try{
            ...
            if($isAdmin){
                return view('livewire.calendar.edit',['agregarEmpleados' => $agregarEmpleados,'citas'=>$this->useDate(),'propinas'=>$this->propinas,'type' => $this->type,'itemSelected'=>$this->itemSelected,'methods' => $this->methods,'totalCart' => $this->totalCart, 'taxCart' => $this->taxCart, 'subtotalCart' => $this->subtotalCart, 'generated_points' => $this->generated_points, 'items'=>$this->items,'times'=>$times,'salonTimes'=>$salonTimes,'pp_cart'=>$this->pp_cart,'categoriasCliente' => $this->listCategories,'total_disccount' => $this->total_disccount,'restante' => $this->rest,'isAdmin' => $isAdmin]);
        ...
        try{
            ...
            if($cust){
                if(isset($cust->tarjetaPuntos)){
                    if($cust->tarjetaPuntos->balance>0 && !$this->inMethods()){
            ...
            if(isset($this->itemSelected)){
                ...
                if($withPnS){
                    foreach($details_service as $detail){
                        $uid = $this->AddItem('servicio',$detail,$detail->servicio ?? servicio::find(99999),1   ,$detail->discount_qty,$detail->iva,$detail->empleado_id,null,null,$detail->discount_type,$detail->current_price,$detail->disccount_price,$detail->generated_points);
    ...
    private function AddProduct($product, $qty = 1,$disccount_percent=0, $ind_iva=0.16,$empleado=NULL,$gross_price=null,$disccount_price=0,$qty_inicial=0,$discount_type="Porcentaje")
    {
        try{
            // iva méxico 16%
            $iva = $ind_iva;
    ...
    public function removeItem($id,$type,$item_id=null)
    {
        try{
            ...
            if($type=='servicio'){
                ...
                $this->cartS  = $this->cartS->reject(function ($service) use ($id, $item_id) {
                    return  $service['id'] === $id || $service['sid'] === $item_id;
                });
    ...
    public function activateCardCust()
    {
        $this->emit('activateCard',$this->customerId);
    }
    ...
    public function storeDate($agendar=0)
    {
        ...
        try{
            ...
            foreach($this->methods as $method) {
                ...
                if($method['paymentMethod']=='1' && $this->rest<0){
            ...
            if (session()->has('returnCustomersView')){
                $this->clearSession(['returnCustomersView']);
        ...
        try{
            ...
            if(isset($empleado->comision)){
                ...
                if(isset($empleado->comision->excepcion_servicio)||isset($empleado->comision->excepcion_cat_servicio)){
                    $precioSinIva=$item['gross_price']-($item['gross_price']*$item['ind_iva']);
    ...
    public function changeWindow($tipo,$cita_id=null)
    {
        try{
            ...
            if($cita_id!=null){
                ...
                if($this->itemSelected->salon_id == Auth::user()->salon_id && $this->itemSelected!==null){
