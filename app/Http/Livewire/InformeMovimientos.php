...
class InformeMovimientos extends Component
{
    ...
    protected $listeners = [
        'refresh' => '$refresh','datesSelected' => 'setDatesFromPeriod',
        'prevDay','dateSelected' => 'setDate','viewDetails','filesDroped',
        'recuperarInfo','editar','updateEmpleado','updateQty','deleteUso',
        'updateIva','removeItem','updatePercentage','loadItems','addNewProduct','cambioDataMethods',
        'newPropina','newMethod','changeTerminalQty','changeQty','StoreCorte','selectAssigment','changeWindow','storeDate' => 'Store','deactivateCheckers'
    ];
    ...
    private function useDate()
    {
        try{
            ...
            if($this->is_interval==false){
                if($this->citasFilter){
                    $citas = cita::with('etiquetas','user','customer','details_product.product','details.servicio','metodosPago.metodoPago','propinas')
                        ->where('salon_id',$this->salon_id)
                        ->whereDate('start',$this->currentDateC)
                        ->orderBy('start', 'desc')
                        ->get();
                }
                if($this->ventasFilter){
                    $ventas = venta::with('user','customer','metodosPago.metodoPago','propinas','details.product')
                        ->where('salon_id',$this->salon_id)
                        ->whereDate('created_at',[$this->currentDateC])
                        ->orderBy('created_at', 'desc')
                        ->get();
                }
                if($this->aperturasFilter){
                    $aperturas = caja_apertura::with('user','corteCaja')
                ...
                if($this->usosFilter){
                    $usos = Material::with('asignacion.date','producto','customer','user','empleado')
                    ->whereHas('user', function ($query) {
                        $query->where('salon_id', $this->salon_id);
                    })
                    ->whereDate('created_at',[$this->currentDateC])
                ...
                if($this->entradasFilter){
                    $entradas = Entrada::with('marca','producto','user')
                    ->whereHas('user', function ($query) {
                        $query->where('salon_id', $this->salon_id);
                    })
                    ->whereDate('created_at',[$this->currentDateC])
                    ...
                    ->get()
                    ->groupBy(function($item) {
                        return $item->updated_at; // Agrupa por fecha (año-mes-día)
                    });
                }
            }else{
                if($this->ventasFilter){
    ...
    private function acumularDescuentos($transacciones)
    {
        foreach($transacciones as $transaccion){
            foreach($transaccion->details as $detail){
                $total_discount = $detail->disccount_price > 0 ? $detail->current_price-$detail->disccount_price : 0;
                ...
                if($discount_qty>0){
                    $qty = $detail->discount_type == 'Porcentaje' ? $total*$discount_qty/100 : $discount_qty;
    ...
    private function consultaRangos()
    {
        try{
            ...
            if($this->is_interval==false){
                ...
                if($this->cortesFilter){
                    $cortes = caja_corte::with('user')
                        ->whereBetween('ganancia',[$this->min ?? 0,$this->max])
                        ->whereHas('user', function ($query) {
                            $query->where('salon_id', $this->salon_id);
                        })
                        ->whereDate('created_at',[$this->currentDateC])
