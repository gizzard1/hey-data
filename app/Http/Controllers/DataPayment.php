...
class DataPayment extends Controller
{
    ...
    public static function updateMethods(Request $request)
    {
        try {
            ...
            $last_status = $sale_id ? $mov->status : $mov['estado'];
            ...
            if ($last_status !== 'Pagada' && $status === 'Pagada') {
                self::updateRewardPoints($sale_id ? $mov->generated_points : cita::select('generated_points')->find($mov['id'])->generated_points, $customer);
            }
    ...
    private static function updateRewardPoints($reward_points, $customer)
    {
        try {
            if ($reward_points > 0 && $customer) {
                $customer->tarjetaPuntos->balance += $reward_points;
                $customer->tarjetaPuntos->save();
            }
        } catch (\Throwable $th) {
            Log::error($th->getMessage());
