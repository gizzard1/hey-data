    ...
    <table>
        ...
        <tbody>
            @foreach($this->info->details as $item)
                ...
                @if($item->disccount_percent>0)
                <tr>
                    <td class="cantidad"><strong>{{ $item->disccount_percent }}%</strong></td>
                    <td class="producto"><strong>{{ $item->product->name }}</strong></td>
                    <td class="precio"><strong>-${{ number_format((($item->disccount_price > 0 ? ($item->disccount_price*$item->quantity)/(100-$item->disccount_percent)*$item->disccount_percent : ($item->current_price*$item->quantity)/(100-$item->disccount_percent)*$item->disccount_percent)),2,'.',',') }}</strong></td>
        ...
        margin: 0 auto;">
            ...
            <tr>Numero de productos: {{ $this->info->items }}</tr>
