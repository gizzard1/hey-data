<script>
function openChecador(){
    $('#modalFingerprint').modal('show')
}
function closeChecador(){
    $('#modalFingerprint').modal('hide')
    onStop()
}
function openFormCreateFingerp(){
    $('#modalFingerprintCreate').modal('show')
}
function closeFormCreateFingerp(){
    $('#modalFingerprintCreate').modal('hide')
    onStop()
}
window.addEventListener('contador', function(event) {
    const intentos = parseInt(event.detail.intentos, 10); // Cast explícito a número
    const max_intentos = parseInt(event.detail.max_intentos, 10); // Cast explícito a número
    const remaining = max_intentos - intentos;
    const message = "Escanee su dedo " + remaining + " " + (remaining === 1 ? "vez" : "veces");
    showMessage(message);
});
...
var FingerprintSdkTest = (function () {
    function FingerprintSdkTest() {
        ...
        this.sdk.onCommunicationFailed = function (e) {
            ...
            showMessage("Fallo de comunicación")
        };
...
function onImageDownload(create=0){
    let imageSrc = localStorage.getItem("imageSrc");
    
    if(imageSrc == "" || imageSrc == null || document.getElementById('imagediv').innerHTML == "") {
        alert("No image to download");
    } else {
        cropImage(imageSrc, function(croppedImageBase64) {
            const canvas = document.getElementById('canvas');
            const ctx = canvas.getContext('2d');

            // Crear una nueva imagen a partir de la base64 para dibujarla en el canvas
            const img = new Image();
            img.onload = function() {
            ...
            applyGaborFilter(croppedImageBase64, function(gaborImageBase64) {
                ...
                img.src = gaborImageBase64;
            });
            processFingerprintPatterns(croppedImageBase64, function(gaborImageBase64) {
                const canvas = document.getElementById('gaborCanvas');
                const ctx = canvas.getContext('2d');
